...
/**
 * @file
 * @brief Just for operator<<(std::ostream & o, hnc::tabular const & t)
 */
...
namespace hnc
{
	...
	public:
		...
		std::string to_string() const
		{
			...
			auto add_line = [&](std::size_t const id_row) -> void
			{
				...
				for (std::size_t column = 0; column < nb_col(); ++column)
				{
					// Detect if merge
					bool merge = false;
					for (long int i = id_row; i >= 0; --i)
					{
						if (m_data[i][column].in_col_merge.nb_merge > id_row - i) { merge = true; break; }
					}
		...
		std::string latex(std::string const & tab = "\t") const
		{
			...
			// For cell merge
			std::vector<long int> skip_hline(nb_col(), 0);
			long int skip_col = 0;
			...
			auto add_line_data = [&](std::vector<cell_t> const & row) -> void
			{
				...
				for (std::size_t i = 0; i < row.size(); ++i)
				{
					...
					if (skip_col <= 0)
					{
						...
						{
							skip_col = cell.in_row_merge.nb_merge;
						...
						{
							skip_hline[i] = cell.in_col_merge.nb_merge;
		...
		std::string html(std::string const & tab = "\t") const
		{
			...
			// For cell merge
			std::vector<long int> skip_tr(nb_col(), 0);
			long int skip_th_or_td = 0;
				...
				for (std::size_t i = 0; i < row.size(); ++i)
				{
					...
					if (skip_th_or_td <= 0)
					{
						...
						if (skip_tr[i] <= 0)
						{
							...
							if (cell.in_col_merge.nb_merge > 1)
							{
								skip_tr[i] = cell.in_col_merge.nb_merge;
								r += " rowspan=\"" + hnc::to_string(cell.in_col_merge.nb_merge) + "\"";
							}
							// In row merge (colspan)
							if (cell.in_row_merge.nb_merge > 1)
							{
								skip_th_or_td = cell.in_row_merge.nb_merge;
...
/**
 * @brief Display a hnc::tabular
 *
 * @param[out] o Out stream
 * @param[in]  v A hnc::tabular
 *
 * @return the out stream
 */
std::ostream & operator<<(std::ostream & o, hnc::tabular const & t)
{
	o << t.to_string();
	return o;
}
