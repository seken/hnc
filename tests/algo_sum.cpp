...
int main()
{
...

	{
		...
		++nb_test;
		{
			double sum = hnc::algo::sum(c);
			std::cout << "- hnc::algo::sum container = " << sum << std::endl;
			nb_test -= hnc::test::warning(sum == sumRef, "hnc::algo::sum container == " + hnc::to_string(sum) + " instead of " + hnc::to_string(sumRef) + "\n");
		}
		++nb_test;
		{
			double sum = hnc::algo::sum(c.begin(), c.end());
			std::cout << "- hnc::algo::sum iterator  = " << sum << std::endl;
			nb_test -= hnc::test::warning(sum == sumRef, "hnc::algo::sum iterator == " + hnc::to_string(sum) + " instead of " + hnc::to_string(sumRef) + "\n");
		}
	}

	{
		...
		++nb_test;
		{
			int sum = hnc::algo::sum(c.begin(), c.begin() + separator);
			std::cout << "- [0 -> " << separator << "[. Reference                = " << sumRef0 << std::endl;
			std::cout << "- [0 -> " << separator << "[. hnc::algo::sum iterator  = " << sum << std::endl;
			nb_test -= hnc::test::warning(sum == sumRef0, "hnc::algo::sum iterator == " + hnc::to_string(sum) + " instead of " + hnc::to_string(sumRef0) + "\n");
		}
		++nb_test;
		{
			int sum = hnc::algo::sum(c.begin() + separator, c.end());
			std::cout << "- [" << separator << ", end[. Reference                = " << sumRef1 << std::endl;
			std::cout << "- [" << separator << ", end[. hnc::algo::sum iterator  = " << sum << std::endl;
			nb_test -= hnc::test::warning(sum == sumRef1, "hnc::algo::sum iterator == " + hnc::to_string(sum) + " instead of " + hnc::to_string(sumRef1) + "\n");
		}
	}

	{
		...
		++nb_test;
		{
			std::string sum = hnc::algo::sum(c);
			std::cout << "- hnc::algo::sum container = " << sum << std::endl;
			nb_test -= hnc::test::warning(sum == sumRef, "hnc::algo::sum container == " + sum + " instead of " + sumRef + "\n");
		}
		++nb_test;
		{
			std::string sum = hnc::algo::sum(c.begin(), c.end());
			std::cout << "- hnc::algo::sum iterator  = " << sum << std::endl;
			nb_test -= hnc::test::warning(sum == sumRef, "hnc::algo::sum iterator == " + sum + " instead of " + sumRef + "\n");
		}
	}

	hnc::test::warning(nb_test == 0, "hnc::algo::sum: " + hnc::to_string(nb_test) + " test fail!\n");
